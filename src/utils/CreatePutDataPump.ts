...
export const createRowPump = async (
  ...
  try {
    ...
    const electricalQuery = `INSERT INTO pump (id, device_id, ${pumpColumns.join(', ')}, created_at) VALUES (?, ?, ${pumpColumns.map(() => '?').join(', ')}, ?)`;
    ...
    for (const field of fileFields) {
      if (files[field] && files[field][0]) {
        ...
        const checkExistingQuery =
          'SELECT * FROM pump_photo WHERE asset_id = ?';
        ...
        if (existingRows.length > 0) {
          // Update query
          photoQuery = `UPDATE pump_photo SET foto${fileIndex} = ?, created_at = ?, user_id = ? WHERE asset_id = ?`;
          photoParams = [newFileName, now, req.body.user_id, newpumpId];
        } else {
          // Insert query
          photoQuery = `INSERT INTO pump_photo (asset_id, foto${fileIndex}, created_at, user_id) VALUES (?, ?, ?, ?)`;
          photoParams = [newpumpId, newFileName, now, req.body.user_id];
        }
